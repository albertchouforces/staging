...
export function MatchingCard({ pairs, onComplete }: MatchingCardProps) {
  ...
  const renderItem = useCallback((item: ParsedItem, isLeft: boolean) => {
    ...
    const stateClasses = isJustMatched
      ...
      : "bg-white hover:bg-blue-50 cursor-pointer border border-gray-800 shadow-sm hover:shadow-md hover:border-black";
    ...
    if (item.type === 'text') {
      return (
        ...
        >
          <div className="font-medium select-none pointer-events-none [&_ul]:list-disc [&_ul]:ml-6 [&_ul]:my-2 [&_ul]:pointer-events-none [&_ol]:list-decimal [&_ol]:ml-6 [&_ol]:my-2 [&_ol]:pointer-events-none [&_li]:mb-1 [&_li]:pointer-events-none [&_*]:pointer-events-none" dangerouslySetInnerHTML={{ __html: item.value as string }} />
